...
<body>
    ...
    <div class="container">
        <h1 class="titulo-categoria">Livros</h1>
        <div class="filtros">
            <a href="vestuario.php" class="btn-filtro <?= empty($_GET['subcategoria']) ? 'ativo' : '' ?>">Todos</a>
            <a href="vestuario.php?subcategoria=romance" class="btn-filtro <?= ($_GET['subcategoria'] ?? '') === 'romance' ? 'ativo' : '' ?>">romance</a>
            <a href="vestuario.php?subcategoria=ficcao" class="btn-filtro <?= ($_GET['subcategoria'] ?? '') === 'ficcao' ? 'ativo' : '' ?>">ficcao</a>
            <a href="vestuario.php?subcategoria=didatico" class="btn-filtro <?= ($_GET['subcategoria'] ?? '') === 'didatico' ? 'ativo' : '' ?>">didatico</a>
            <a href="vestuario.php?subcategoria=hq" class="btn-filtro <?= ($_GET['subcategoria'] ?? '') === 'calça' ? 'hq' : '' ?>">HQ</a>
            <a href="vestuario.php?subcategoria=biografia" class="btn-filtro <?= ($_GET['subcategoria'] ?? '') === 'biografia' ? 'ativo' : '' ?>">biografia</a>
            <a href="vestuario.php?subcategoria=terror" class="btn-filtro <?= ($_GET['subcategoria'] ?? '') === 'terror' ? 'ativo' : '' ?>">terror</a>
        </div>
            ...
            <?php if (count($anuncios) > 0): ?>
                <?php foreach ($anuncios as $anuncio): ?>
                    <div class="col-4 produto">
                        ...
                        <button class="btn">Adicionar no carrinho</button>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
